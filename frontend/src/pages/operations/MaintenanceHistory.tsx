      ...
      <section className="bg-bg-surface border border-border-subtle rounded-lg p-6 flex flex-col min-h-[520px]">
        <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4 mb-5">
          <h3 className="text-lg font-sans font-medium text-on-surface flex items-center gap-2 shrink-0">
            <History className="w-5 h-5 text-primary" /> Maintenance History
          </h3>
          <div className="flex flex-col sm:flex-row gap-3 flex-1 xl:justify-end">
            <div className="relative w-full sm:max-w-md">
              ...
              <input
                type="text"
                placeholder="Search title, equipment, site, assignee, or report..."
                value={searchTerm}
        ...
        <div className="flex-1 overflow-y-auto space-y-3 pr-2">
          {isLoading ? (
            <div className="text-on-surface-variant text-sm">Loading maintenance history...</div>
          ) : history.length === 0 ? (
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <Badge status={statusForState(task.report?.currentState)}>
                      {task.report?.currentState || 'Completed'}
                    </Badge>
                  </div>
                  <h4 className="font-sans font-medium text-on-surface mt-2">{task.title}</h4>
                  <p className="text-xs text-on-surface-variant mt-1">
                    {task.equipment?.room?.site?.name || 'Site'} / {task.equipment?.name || 'Equipment'} / {task.equipment?.room?.name || 'Room'}
                  </p>
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  <div className="text-xs text-on-surface-variant text-right">
                    <div>{new Date(task.report?.createdAt || task.scheduledDate).toLocaleString()}</div>
                    <div>Scheduled {new Date(task.scheduledDate).toLocaleDateString()}</div>
                  </div>
                  {canDelete && (
                    <button
                      type="button"
                      ...
                      aria-label={`Remove maintenance record ${task.id.substring(0, 8)}`}
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-3 mt-4 text-sm">
                <div className="bg-bg-surface border border-border-subtle rounded-md p-3">
                  <p className="text-xs uppercase font-mono text-on-surface-variant mb-1 flex items-center gap-1.5">
                    <Wrench className="w-3.5 h-3.5" /> Action Taken
                  </p>
                  <p className="text-on-surface whitespace-pre-wrap">{task.report?.actionTaken || 'No action recorded.'}</p>
                </div>
                <div className="bg-bg-surface border border-border-subtle rounded-md p-3">
                  <p className="text-xs uppercase font-mono text-on-surface-variant mb-1 flex items-center gap-1.5">
                    <CheckCircle className="w-3.5 h-3.5" /> Current State
                  </p>
                  <p className="text-on-surface">{task.report?.currentState || 'Completed'}</p>
                </div>
                <div className="bg-bg-surface border border-border-subtle rounded-md p-3">
                  <p className="text-xs uppercase font-mono text-on-surface-variant mb-1 flex items-center gap-1.5">
                  ...
                  <p className="text-on-surface">
                    {task.report?.submitter ? `${task.report.submitter.firstName} ${task.report.submitter.lastName}` : task.assignee ? `${task.assignee.firstName} ${task.assignee.lastName}` : 'Unknown'}
                  </p>
                </div>
              </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-bg-surface border border-border-subtle rounded-lg w-full max-w-md shadow-xl">
            <div className="flex justify-between items-center p-4 border-b border-border-subtle">
              <h3 className="text-lg font-sans font-medium text-on-surface">Remove Maintenance Record</h3>
            ...
            <div className="p-4 space-y-4">
              ...
              <p className="text-sm text-on-surface-variant">
                {deleteIntent.title} will be removed from maintenance history.
              </p>
